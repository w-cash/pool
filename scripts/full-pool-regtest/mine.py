...
for index in range(args.blocks):
    ...
    proof = h.command(f'continuation-proof-{previous["wec"]+1:04}', [args.miner,
        'zip301-mine', '127.0.0.1:18237', worker['mining_username'], '256', '0'],
        env=env, timeout=900)
    if json.loads(proof).get('result') != 'accepted':
        raise RuntimeError('real ZIP301 submission was not accepted')
    for name in ('wec', 'zec'):
