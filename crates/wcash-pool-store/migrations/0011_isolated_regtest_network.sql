-- Regtest is available only in explicitly feature-enabled local runtimes.
-- Existing Testnet deployments retain their full 48-hour destination hold.
ALTER TABLE public.deployments DROP CONSTRAINT deployments_network_check;
ALTER TABLE public.deployments ADD CONSTRAINT deployments_network_check
    CHECK (network IN ('testnet', 'mainnet', 'regtest'));
ALTER TABLE public.payout_destinations DROP CONSTRAINT payout_destinations_network_check;
ALTER TABLE public.payout_destinations ADD CONSTRAINT payout_destinations_network_check
    CHECK (network IN ('testnet', 'mainnet', 'regtest'));
...
BEGIN
    IF p_deployment_id IS NULL
        ...
        OR p_network NOT IN ('testnet', 'regtest')
...
BEGIN
    ...
    SELECT network INTO v_deployment_network
      FROM public.deployments
     WHERE id = p_deployment_id;
    IF NOT FOUND OR v_deployment_network NOT IN ('testnet', 'regtest') THEN
