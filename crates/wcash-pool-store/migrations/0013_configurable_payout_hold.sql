-- Keep the original v1 routine stable for older clients while allowing the
-- reviewed runtime policy to choose a bounded Testnet hold. The public role
-- still cannot write payout rows directly; this SECURITY DEFINER routine is
-- the only mutation path and the database clock remains authoritative.
CREATE FUNCTION public.configure_payout_destination_v2(
