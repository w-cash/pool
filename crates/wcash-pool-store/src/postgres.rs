...
// maturity event. Mainnet emits maturity events in periodic batches that can
// leave the wallet ahead of the projected ledger for about five minutes, so
// allow a bounded ten-minute catch-up window without permitting a payout from
// a mismatched checkpoint. An unchanged mismatch still freezes the chain after
// this window.
const WALLET_RECONCILIATION_MISMATCH_GRACE_SECS: u64 = 10 * 60;
